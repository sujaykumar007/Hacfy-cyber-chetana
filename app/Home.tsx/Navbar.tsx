    ...
    return (
        ...
        <nav className={`w-full pt-10 lg:pt-4 fixed px-5 lg:px-16 xl:px-[10%] py-4 flex items-center justify-between z-50 transition-all duration-300 ${isScroll ? "bg-white backdrop-blur-lg shadow-md" : "bg-white"}`}>   
            ...
            <div className={`flex items-center gap-4 ${isScroll ? "" : "text-black"}`}>
                ...
                <Link href="/Pledge" className={`hidden lg:flex items-center gap-3 px-15 py-2 border font-medium text-lg border-gray-500  rounded-xl ml-4 relative overflow-hidden group transition-colors duration-500  hover:text-black ${isScroll ? "text-black" : "text-black"}`}>
                    <span className="relative z-10">Pledge</span>
                    <span className="absolute inset-0 bg- scale-x-0 origin-left transition-transform duration-1000 ease-out group-hover:scale-x-100"></span>
                </Link>
